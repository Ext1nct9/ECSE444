...
// Buffer count for setting values for DAC
uint16_t buf_count = 0;

// Sine wave buffer
uint16_t dac_buffer_sin[(uint16_t) SAMPLE_RATE / FREQUENCY];

// Prescaler values
uint16_t periods[3] = {2214, 1814, 1414};

// Current tone value, modified using an ISR so without volatile the compiler might optimize accesses assuming that its value wont change outside the main program flow. Could affect if the ISR modifies current_tone and the main logic relies on its updated value.
...
int main(void)
{
 ...
 for (uint16_t i = 0; i < period; ++i) {
  ...
  for (uint16_t j = 0; j < period; j++) {
	  float phase = (float)(j) * 2 * 3.14 * FREQUENCY / SAMPLE_RATE;
	  float new_A = amplitude / 3;
	  dac_buffer_sin[j] = new_A * arm_sin_f32(phase) + new_A;
  }

  uint16_t debug_sin[(uint16_t) SAMPLE_RATE / FREQUENCY];
  for (uint16_t j = 0; j < period; j++) {
	  debug_sin[j] = dac_buffer_sin[j];
  }

  //HAL_DAC_Start(&hdac1, DAC_CHANNEL_1);
  HAL_DAC_Start_DMA(&hdac1, DAC_CHANNEL_1, (uint16_t*) dac_buffer_sin, (uint16_t) SAMPLE_RATE/FREQUENCY, DAC_ALIGN_12B_R);
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
	if (GPIO_Pin == myButton_Pin) {
		...
		// Set new prescaler value and restart the timer.
		HAL_TIM_Base_Stop_IT(&htim2);
		htim2.Init.Prescaler = periods[current_tone];
		if (HAL_TIM_Base_Init(&htim2) != HAL_OK){
		  Error_Handler();
		}
		HAL_TIM_Base_Start_IT(&htim2);
